...
- `Course_Weight-Optimizer/global_state.json`：年级人数、课程容量和当前参与人数快照。
...
写 JSON 前必须展示一份简短摘要，至少包括：课程、容量、当前人数、学生意愿分数、建议投权、SAFE/COMP/OUT 标签，以及预算是否超限。明确区分“截图中读到的事实”和“模型计算出的建议”，并等待学生明确确认。学生没有确认、回答含糊或数据不完整时，不写入文件。

### 5. 生成、校验并运行 JSON

得到确认后，Agent 才能把结构化结果写入 `desired_courses.json` 和 `global_state.json`，并检查：课程标识一致、容量和人数为非负整数、年级人数为正整数、意愿分数为正数、预算和最低投权仍遵守项目规则。随后运行程序，把终端结果和任何警告原样告诉学生。
