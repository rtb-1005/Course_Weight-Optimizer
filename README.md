...
程序运行时使用两个 JSON 文件：`desired_courses.json`描述你想选哪些课以及每门课对你的相对重要性（偏好强度），`global_state.json`描述当前全局状态（年级人数、所有课程的容量、当前参与人数快照和已确认的课程冲突）。推荐通过 Agent 对话或选课页面截图提供信息，由 Agent 在确认后生成文件；学生不需要手动编辑 JSON。程序运行后会给出每门目标课的建议投权值，并将目标课标注为三类：
...
`conflicts` 是已确认的硬冲突课程对；缺省或为空数组表示当前没有已确认冲突。每个课程标识都必须出现在 `courses` 中，重复和反向重复会被自动合并。
...
对目标学生，设目标课程集合为 $J$，在每门课上的投标为 $b_j$。制度约束可以写为：
...
> 在这里，有一个小小的逻辑谬误，因为一门课可能因为受欢迎而选的多，另一门课可能因为种种原因(eg. 课程难度/教师抽象程度/考核指标) 没几个人选，我这里简单的用热度分析(内心OS belike: 这门课选的人太多了，我可不投了！) 来简单粗暴的说明。我想说的是，下一时刻的变化其实是服从一个偏微分方程的（类似捕食者/猎物）方程，每门课之间都互相制约。可以写成一个LV-equation的变体，也就是说在课程的基础喜欢指数(刚才说的种种原因)可以写成一个基础系数*转换矩阵的偏微分方程形式。这个振动就过于复杂，虽然是一个时序系统，但是维度过多十分难以数值解。
> 不过，我十分十分希望看到后面的PR，有人对这里的方法加以改进！
...
这个分支对应一个关键事实：**当课程终局不满员时，额外投权不产生边际收益，应当把预算节省给真正拥挤的课程**。同时，由于我们取的是“最坏情景”的判定，它也在一定程度上回应了“对手也不傻、也会只投入场券导致人数上升”的担忧：只要最拥挤情景下仍不满员，策略就稳健。

### 4) 竞争性课程：从竞赛成功函数到可微概率分析
...
课程冲突不能由模型根据课程名称猜测，必须来自学生明确说明、学校规则或经过学生确认的课表截图。Agent 在询问偏好和投权之前，应主动问：

1. 哪两门课不能同时选择？
2. 冲突来自上课时间，还是学校明确规定？
3. 这是绝对不能同时选的硬冲突，还是可以换班、调课的条件冲突？
4. 如果一组课程最多只能选一门，这个限制涉及哪些课程？

只有学生确认的硬冲突才写入 `global_state.json`。每条冲突由两个课程标识组成：
...
如果一组课程 $H=\{j_1,\ldots,j_q\}$ 中最多只能选一门，Agent 会将其展开为两两冲突：

$$
E_H = \bigl\{\{j_a,j_b\}: 1\leq a<b\leq q\bigr\}.
$$
...
把目标课程看成无向冲突图 $G=(J,E)$：课程是顶点，若课程 $i$ 与课程 $j$ 不能同时选择，则 $\{i,j\}\in E$。定义二元变量 $x_j$ 表示是否进入课程 $j$：
...
因此，可选择的课程集合不再是任意子集，而必须属于冲突图的独立集族：
...
S^*
=
\arg\max_{\substack{S\in\mathcal{I}(G)\\|S|b_{\min}\leq W}}
\left[
...
$$
\sum_{j\in S\cap J_{\mathrm{COMP}}} b_j
+
|S\cap J_{\mathrm{SAFE}}|b_{\min}
\leq W,
\qquad
b_j\geq b_{\min}.
$$

当 $S\cap J_{\mathrm{COMP}}\neq\varnothing$ 时，代理效用对竞争性课程投权单调递增，因此最优解会用完剩余预算，上式中的不等号取等号；若集合里只有 SAFE 课程，未使用预算可以保留。

当前实现先枚举满足预算和冲突约束的可行课程集合，再对每个集合执行原有的水位式分配，最后选择代理效用最高的方案。这样，冲突改变的是“哪些课程可以共同进入”，而不是给某门课简单乘一个经验折扣。

一般冲突图上的最大独立集问题在最坏情况下是指数复杂度；本项目的精确枚举适合学生实际会比较的有限候选集（本实验覆盖到 12 门）。如果候选课程非常多，Agent 应先帮助学生缩小候选集合，并在后续版本中引入分支定界或整数规划求解器，而不是静默返回未经验证的近似组合。

### 3) 为什么任意课程数量都会受到影响

假设旧算法最终选择了 $m$ 门课，并把任意一对课程发生硬冲突的概率记为 $p$。在用于敏感性分析的独立边模型中，$m$ 门课程一共有 $\binom{m}{2}$ 个课程对，因此旧方案完全无冲突的概率为：
...
这一定量关系适用于任意 $m\geq 2$。它不是对真实冲突概率的估计，而是说明：即使单个课程对的冲突概率很低，课程对数量也会按 $m^2$ 的量级增长。

| 旧方案选择课程数 $m$ | $p=1\%$ | $p=5\%$ | $p=10\%$ |
|---:|---:|---:|---:|
| 3 | 3.0% | 14.3% | 27.1% |
| 5 | 9.6% | 40.1% | 65.1% |
| 8 | 24.5% | 76.2% | 94.8% |
...
因此，容斥关系不是只在“课程很多”时才重要；只要旧方案选择两门以上课程，就存在违反硬约束的可能，而风险会随课程数快速累积。

### 4) 多课程数量 Monte Carlo

为了避免结论依赖仓库里的五门课示例，分析脚本另外生成了 $n\in\{3,5,8,10,12\}$ 门候选课的合成场景。每个 $n$ 使用 8 个随机种子；每个种子在 $p\in\{5\%,10\%,20\%\}$ 下生成 40 张冲突图，因此每个 $(n,p)$ 组合包含 320 次模拟，共 4,800 张冲突图。
...
![课程容斥敏感性分析](figures/conflict_sensitivity.png)

图 A 给出任意旧方案规模 $m$ 的解析失效概率；图 B、C 是跨课程数量的 Monte Carlo 均值，以及按随机种子聚合得到的 95% 置信区间；图 D 表明冲突数量相近时，链、星形和完全互斥等拓扑仍会产生不同损失；图 E 展示仓库样例中单条冲突边的边际影响。完整矢量图见 [`figures/conflict_sensitivity.pdf`](figures/conflict_sensitivity.pdf)。

这组实验保持其他学生的参与人数快照不变，只隔离“硬冲突使课程组合不可行”的直接效应。若其他学生也会因时间冲突迁移到替代课程，竞争人数会内生变化，需要进一步加入动态迁移模型；当前结果不应解释为完整市场均衡。

### 5) 复现实验

核心优化器仍然只依赖 Python 标准库。绘图脚本额外需要 Matplotlib：
...
脚本会重新生成 300 DPI PNG 和矢量 PDF。固定随机种子保证结果可复现。
