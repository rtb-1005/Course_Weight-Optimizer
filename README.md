...
$$
b_j^{*} = b_{\min}+\max\\{0,\thickspace(\alpha_j+\varepsilon)\ln\frac{u_j}{\nu(\alpha_j+\varepsilon)}-b_{\min}\\}, \quad j\in S,
$$
...
$$
E_H = \\{(j_a,j_b)\mid 1\leq a<b\leq q\\}.
$$
...
$$
x_i+x_j\leq 1
\qquad \text{for every conflict pair }(i,j)\in E.
$$
...
$$
\mathcal{I}(G) =
\\{S\subseteq J\mid \text{for every }(i,j)\in E,\ \neg(i\in S\land j\in S)\\}.
$$
...
$$
S^* =
\mathrm{arg\thinspace max}_{S\in\mathcal{I}(G),\thickspace|S|b_{\min}\leq W}
\left[
